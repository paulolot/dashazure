...
function renderDeliveriesByCollaboratorTasks(filteredWIs) {
  ...
  let legendHtml = `<div style="position: absolute; right: 0; top: 25px; width: 260px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 0.8rem;">`;
  ...
  try {
    ...
    const renderRows = (page) => {
      ...
      pageTasks.forEach(t => {
        ...
        let safeTitle = t.Title ? String(t.Title).replace(/"/g, '&quot;') : '';
        
        html += `
          <tr>
            <td><a href="${url}" target="_blank" style="color: var(--color-primary); text-decoration: none; font-weight: 500;">${t.Id}</a></td>
            <td style="max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${safeTitle}">${t.Title || ''}</td>
