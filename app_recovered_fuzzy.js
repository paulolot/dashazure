...
function getFilteredWorkItems() {
  ...
  return g_raw.workItems.filter(wi => {
    ...
    const isClosed = wi.State === 'Closed' || wi.State === 'Resolved' || wi.BoardColumn === 'Concluído' || wi.BoardColumn === 'Pronto pra Release';
    if (isClosed) {
  ...
  if (filtered.length === 0 || filtered.length <= itemsPerPage) {
    ...
    if (btnNext && !btnNext.disabled) {
      btnNext.addEventListener('click', () => {
        state.page++;        processAndRenderGridGeneric(tableId, gridKey, mappedRows, renderRowFn, onCountChange);
      });
    }
  }
}
...
function renderOverview(filteredWIs) {
  ...
  // Separations
  const openWIs = filteredWIs.filter(wi => wi.State !== 'Closed' && wi.State !== 'Removed' && wi.BoardColumn !== 'Concluído' && wi.BoardColumn !== 'Ideias' && wi.BoardColumn !== 'Removed');
...
function renderDeliveries(filteredWIs) {
  ...
  if (dateRangeVal === 'all') {
    ...
    startDate = new Date(today.getFullYear(), today.getMonth() - 1, 1);
    endDate = new Date(today.getFullYear(), today.getMonth(), 0);    } else if (dateRangeVal === 'this-month') {
      startDateMs = new Date(today.getFullYear(), today.getMonth(), 1).getTime();
    } else if (dateRangeVal === 'last-month') {
      startDateMs = new Date(today.getFullYear(), today.getMonth() - 1, 1).getTime();
      endDateMs = new Date(today.getFullYear(), today.getMonth(), 0, 23, 59, 59).getTime();
...
function renderCapacity(filteredWIs) {
  ...
  const activeWIs = capacityWIs.filter(wi => wi.State !== 'Closed' && wi.State !== 'Removed' && wi.BoardColumn !== 'Concluído' && wi.BoardColumn !== 'Ideias' && wi.BoardColumn !== 'Removed');
...
function renderQuality(filteredWIs) {
  ...
  const bugsOpen = pageBugs.filter(b => b.State !== 'Closed' && b.BoardColumn !== 'Concluído');
  ...
  const pageBugsOpen = pageBugs.filter(b => b.State !== 'Closed' && b.BoardColumn !== 'Concluído');
